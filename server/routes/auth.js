...
router.get("/logout", async (req, res) => {
	await Util.getAuthToken(req).then((e) => e.deleteOne());

	res.cookie(Util.AUTH_COOKIE_NAME, "", Util.AUTH_COOKIE_UNSET);
	res.status(200).send(RESPONSE.OK);
});
...
router.get("/user", async (req, res) => {
	...
	//console.log(user);
	if (user == null) res.status(200).send({ message: "new" });
	else if (user != null) res.status(200).send(user);
	else res.status(404).send(RESPONSE.NOT_FOUND);
});
