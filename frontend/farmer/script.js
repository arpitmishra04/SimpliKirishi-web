let items, createNewItem, openOrders, closedOrders;
...
async function loadItems() {
	...
	if (r.status != 200) console.error(data);

	for (let p of data.products) items.appendChild(createItemRow(p));
}
...
function addOrderRow(p) {
	...
	if (p.isOpen) openOrders.appendChild(tr);
	else closedOrders.appendChild(tr);
