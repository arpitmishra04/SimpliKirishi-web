let phone, otp, resendOtp, submit;
...
window.addEventListener("load", (e) => {
	...
	resendOtp.addEventListener("click", (e) => {
		if (checkInputs()) getOtp(phone.value);
	});
});
...
async function handleSubmit(e) {
	...
	if (state == "phone") {
		...
		if (authResponse.status == 200) {
			...
			state = "done";
		} else displayError("#OtpWrapper", "Invalid or expired OTP");
	}
}
...
function displayError(inputWrapper, message) {
	if (typeof inputWrapper == "string") inputWrapper = get(inputWrapper);
